...
class JobState:
    ...
    # DVhub fork (2026-06-28): slot-aligned scheduling marker — the wall-clock
    # slot index this job last fired in (-1 = never). Only the ems/interval job
    # uses it (see is_due) so its run snaps to the wall-clock 15-min grid
    # (:00/:15/:30/:45) instead of drifting on a monotonic "since last run" timer.
    last_run_slot: int = -1
    ...
    def is_due(self) -> bool:
        ...
        # wall-clock slot: when the current slot index is newer than the one we
        # last fired in. int(wall_now // interval) is epoch-aligned, so at
        # interval=900 the boundaries are :00/:15/:30/:45 local (Berlin = whole-hour
        # offset). Overrun on weak hardware needs NO extra logic: tick() already
        # skips a job while is_running, so a run that spills past the next boundary
        # simply makes the loop fire at the following free boundary — the cadence
        # self-halves to 30 min (then 60) exactly as intended.
        if self.interval_attr == "ems/interval":
            current_slot = int(time.time() // interval)
            return current_slot > self.last_run_slot
...
class RetentionManager:
    ...
    async def tick(self) -> None:
        ...
        for job in due:
            # DVhub fork: mark the wall-clock slot for the slot-aligned ems job at
            # LAUNCH (not completion) so the same slot can't re-fire on the next
            # 5-s tick while the ~6-min optimization is still running.
            if job.interval_attr == "ems/interval":
                iv = job.interval()
                if iv:
                    job.last_run_slot = int(time.time() // iv)
