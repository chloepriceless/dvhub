...
class JobState:
    ...
    # DVhub fork (2026-06-28): WALL-CLOCK time.time() of the last fire, used ONLY
    # by the slot-aligned ems/interval job (see is_due). Stored as a TIMESTAMP, not
    # a slot index, so both slot boundaries are recomputed with the CURRENT
    # interval at compare time — robust to ems/interval changing between runs (e.g.
    # an EOS restart momentarily reverts it to the 300 s default before config-sync
    # re-asserts 900). 0.0 = never fired.
    ...
    def is_due(self) -> bool:
        ...
        if interval is None:
            return False
        # DVhub fork (2026-06-28): the energy-management optimization is aligned to
        # the WALL-CLOCK slot grid so a fresh plan is produced at the start of each
        # slot (and steers the following slots), instead of free-running on a
        # monotonic timer that drifts ~1 min/cycle across the slots. Fire once per
        # wall-clock slot: when the current slot is newer than the one we last fired
        # in. Both indices are computed with the CURRENT interval (int(t // interval)
        # is epoch-aligned → :00/:15/:30/:45 local for interval=900, Berlin =
        # whole-hour offset). Comparing TIMESTAMPS (not a stored index) makes this
        # robust to interval changes between runs — a stored slot index baked in the
        # old interval and broke the comparison permanently (fixed 2026-06-28).
        # Overrun on weak hardware needs NO extra logic: tick() already skips a job
        # while is_running, so a run that spills past the next boundary makes the
        # loop fire at the following free boundary — the cadence self-halves to 30
        # (then 60) min exactly as intended.
        if self.interval_attr == "ems/interval":
            if self.last_run_wall <= 0.0:
                return True  # never fired → run now
            return int(time.time() // interval) > int(self.last_run_wall // interval)
...
class RetentionManager:
    ...
    async def tick(self) -> None:
        ...
        for job in due:
            # DVhub fork: stamp the wall-clock fire time of the slot-aligned ems job
            # at LAUNCH (not completion) so the same slot can't re-fire on the next
            # 5-s tick while the ~6-min optimization is still running.
            if job.interval_attr == "ems/interval":
                job.last_run_wall = time.time()
