...
function buildFieldDefinitions() {
  const fields = [
    ...
    },
    {
      ...
      groupLabel: 'PV-Export-Modus (Ladeverhinderung)',
      groupDescription: 'Verhindert Akkuladung durch dynamischen Grid Setpoint der die gesamte PV-Leistung (DC + AC) ins Netz einspeist.',
      path: 'dcExportMode.batteryEfficiencyPct',
      label: 'Speicher-Wirkungsgrad (%)',
      type: 'number',
      min: 50,
      max: 100,
      help: 'Nur bei Volleinspeisung (kein Akku-Laden im Slot) und aktivem Hausverbrauch-Abzug: Der Eigenverbrauch zieht real noch Leistung aus dem Akku (DC-AC-Wandlungsverlust), die mit dem Verbrauch steigt/fällt. Es wird ein DYNAMISCHER Aufschlag = Hausverbrauch × (100 − Wirkungsgrad)/100 zusätzlich vom Export zurückgehalten, damit PV den Akku-Beitrag mitdeckt und der Akku-Nettostrom wirklich ~0 A bleibt. Standard: 92 % (≈ 8 % Verlust, bei 5 kW Verbrauch ≈ 400 W). 100 % = kein Aufschlag.'
    },
    {
      section: 'system',
      group: 'dcExportMode',
    ...
    // --- Steuerung-Gruppen: Optimierung, Akku-Grenzen, Einspeisung, Rechtliche Schalter ---
    {
      section: 'schedule',
      group: 'optimization',
