...
test('loadConfigFile: persisted victron.alarms unit-ids SURVIVE the profile merge (live-deploy gap)', () => {
  ...
  // 29.07.2026: NICHT mehr `true`. Der Default steht jetzt auf AUS (die
  // Alarm-Abfrage liest zwei weitere Geräte auf derselben Verbindung und hat
  // auf der Betreiber-Anlage den Steuerpfad ausgebremst). Und: der
  // alarms-Block eines Profils ist ohnehin WIRKUNGSLOS — applyManufacturerProfile
  // ersetzt victron.alarms komplett durch den persistierten Wert, und der wird
  // immer aus createDefaultConfig befüllt. Deshalb wurde er aus dem
  // ausgelieferten hersteller/victron.json entfernt.
  assert.equal(eff.victron.alarms.enabled, false, 'Vorgabe kommt aus createDefaultConfig, nicht aus dem Profil');
...
test('loadConfigFile: no persisted alarms → profile default (feature present but inactive)', () => {
  ...
  assert.equal(eff.victron.alarms.enabled, false, 'Default AUS (Christin 29.07.2026)');
  assert.equal(eff.victron.alarms.vebusUnitId, null, 'no unit-ids → inactive (no banner), safe default');
});

// Der alarms-Block im Profil ist wirkungslos — festgehalten, damit ihn niemand
// wieder einträgt und sich auf ihn verlässt.
test('ein alarms-Block im Herstellerprofil bleibt wirkungslos', () => {
  const eff = withProfileFixture(
    { host: 'cerbo.local' },
    { enabled: true, pollIntervalMs: 9999, timeoutMs: 42, vebusUnitId: 229, batteryUnitId: 225 }
  );
  assert.equal(eff.victron.alarms.enabled, false);
  assert.equal(eff.victron.alarms.pollIntervalMs, 30000, 'Profilwert 9999 kommt nicht durch');
  assert.equal(eff.victron.alarms.vebusUnitId, null, 'Profil-Unit-IDs kommen nicht durch');
});
