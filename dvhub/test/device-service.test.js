...
// --- Regression: ctx.db is a LAZY GETTER, assigned late (Inc 2 Vorarbeit) ---
//
// server.js defines `get db() { return dbPool; }` and only assigns dbPool inside
// createTelemetryStoreIfEnabled() (server.js:1425) — long AFTER the device service
// is constructed (server.js:1067). The factory used to destructure `db` from ctx,
// which evaluated the getter once at construction time and froze it to null.
// Effect on prod .66: device_readings was never created and no reading was ever
// persisted, so /api/devices/:id returned history:[] forever (silently).
describe('Device Service — late-bound ctx.db (lazy getter)', () => {
  it('picks up a db that only exists after construction', async () => {
    let lateDb = null;                       // dbPool is still null at factory time
    const queries = [];
    const ctx = {
      getCfg: () => ({ devices: [] }),
      pushLog: () => {},
      get db() { return lateDb; },           // exactly the server.js shape
    };

    const service = createDeviceService(ctx, null);

    // ...pool appears only now, mirroring createTelemetryStoreIfEnabled()
    lateDb = {
      query(sql, params) { queries.push({ sql, params }); return Promise.resolve({ rows: [] }); },
    };

    await service.start();

    const schemaQuery = queries.find(q => q.sql.includes('CREATE TABLE IF NOT EXISTS device_readings'));
    assert.ok(schemaQuery, 'schema must be created even though ctx.db was null at construction time');

    await service.close();
  });

  it('persists readings through the late-bound db', async () => {
    let lateDb = null;
    const queries = [];
    const ctx = {
      getCfg: () => ({ devices: [] }),
      pushLog: () => {},
      get db() { return lateDb; },
    };
    const service = createDeviceService(ctx, null);
    lateDb = {
      query(sql, params) { queries.push({ sql, params }); return Promise.resolve({ rows: [] }); },
    ...
    await service.start();
    await service._persistReadings();
    // No devices configured -> no INSERT, but the call must not bail out on a
    // frozen-null db before it even looks at the readings.
    assert.ok(queries.some(q => q.sql.includes('device_readings')), 'db was reached');
    await service.close();
  });
});
