...
const OPTS = resolveFreezeOptions({
  // Beta-Gate (29.07.2026): der Waechter ist ein Feldtest und laeuft nur im
  // Bleeding-Edge-Kanal — siehe beta-features.js und test/beta-gate.test.js.
  updateChannel: 'dev',
...
test('ohne bekannte Akkukapazität bleibt Detektor B aus', () => {
  const opts = resolveFreezeOptions({ updateChannel: 'dev', victron: { freezeWatchdog: { freezeMs: 60000, minSamples: 10 } } });
...
function makeWatchdog({ transportType = 'modbus' } = {}) {
  ...
  const ctx = {
    state,
    getCfg: () => ({
      updateChannel: 'dev',   // Beta-Gate: der Waechter laeuft nur im Bleeding-Edge-Kanal
