...
describe('TeslaMate DB historisation + restart-seed', () => {
  ...
  // ── Inc 2: per-field timestamps for the load-separation freshness gate ──
  //
  // `lastUpdateAt` is global — every topic bumps it — so it cannot tell whether
  // charger_power specifically is still valid. charger_power was measured latching
  // for up to 22.3 h while battery_level kept ticking; a gate built on lastUpdateAt
  // would read "fresh" the whole time and subtract phantom load from the house curve.

  it('getFieldUpdatedAt() stamps each field independently, unlike the global lastUpdateAt', async () => {
    const ctx = makeMockCtx();
    const sub = createTeslamateSubscriber(hub, ctx);
    await sub.start();

    hub._deliver('teslamate/cars/2/charger_power', '10.7');
    const afterPower = sub.getFieldUpdatedAt().chargerPower;
    assert.ok(afterPower instanceof Date, 'charger_power stamped on receipt');

    // A different, fast-moving topic arrives. It must NOT refresh charger_power.
    hub._deliver('teslamate/cars/2/battery_level', '61');
    const stamps = sub.getFieldUpdatedAt();
    assert.equal(
      stamps.chargerPower.getTime(), afterPower.getTime(),
      'battery_level must not launder charger_power fresh'
    );
    assert.ok(stamps.batteryLevel instanceof Date, 'battery_level has its own stamp');
    assert.equal(stamps.geofence, undefined, 'a never-received field has no stamp at all');
  });

  it('a field never received has no stamp, so the gate cannot default it to fresh', async () => {
    const ctx = makeMockCtx();
    const sub = createTeslamateSubscriber(hub, ctx);
    await sub.start();
    assert.deepEqual(Object.keys(sub.getFieldUpdatedAt()), []);
  });

  it('the seed stamps fields with the SAMPLE ts, so a restart cannot wash a latched value fresh', async () => {
    // A previous run left a 22.3 h old charger_power sample (the measured worst
    // case). If the seed stamped it now(), the freshness gate would accept it right
    // after every restart and subtract ~10.7 kW that is not being drawn.
    const staleIso = '2026-07-28T14:00:00.000Z';
    const store = makeMockStore({ seedRows: [
      { key: 'tesla_charger_power', ts: staleIso, value: 10.7, unit: null, resolution: 1 },
      { key: 'tesla_battery_level', ts: '2026-07-29T11:59:00.000Z', value: 61, unit: null, resolution: 1 }
    ] });
    const ctx = makeMockCtx({ telemetryStore: store });
    const sub = createTeslamateSubscriber(hub, ctx);
    await sub.start();
    ...
    const stamps = sub.getFieldUpdatedAt();
    assert.equal(
      stamps.chargerPower.toISOString(), staleIso,
      'seeded stamp is the sample ts, not the restart time'
    );
    assert.notEqual(
      stamps.chargerPower.getTime(), stamps.batteryLevel.getTime(),
      'each seeded field keeps its own age'
    );
  });
